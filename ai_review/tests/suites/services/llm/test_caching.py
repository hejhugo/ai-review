...
from ai_review.services.prompt.schema import SYSTEM_PROMPT_CACHE_BOUNDARY
...
class TestAnthropicCaching:
    ...
    def test_gate_uses_prefix_length_not_total_when_boundary_present(self, monkeypatch):
        # A 5-char stable prefix + 5000-char variable would clear the
        # threshold on total length, but the actual cacheable block is too
        # small for Anthropic's cache floor. Gate must compare against
        # the prefix, not the whole prompt.
        make_claude_config(cache_enabled=True, min_chars=100, monkeypatch=monkeypatch)
        tiny_prefix = "short"
        long_variable = "x" * 5000
        prompt_system = f"{tiny_prefix}{SYSTEM_PROMPT_CACHE_BOUNDARY}{long_variable}"
        result = _build_system(prompt_system)
        # Boundary stripped, no cache_control blocks attached.
        assert isinstance(result, str)
...
class TestOpenAIDefaultOffParity:
    ...
    async def test_enabled_above_floor_uses_instructions_field(self, monkeypatch):
        ...
        client = OpenAILLMClient()
        # Cached prefix must clear min_chars on its own; a long total prompt
        # without a boundary marker means cached_prefix="" and falls back to
        # the legacy system-message shape.
        long_prefix = "x" * 100
        variable = "stage-specific rules"
        prompt_system = f"{long_prefix}{SYSTEM_PROMPT_CACHE_BOUNDARY}{variable}"
        result = await client.chat("user prompt", prompt_system)

        request = fake.requests[0]
        assert request.instructions == long_prefix
        assert len(request.input) == 2
        assert request.input[0].role == "system"
        assert request.input[0].content == variable
        assert request.input[1].role == "user"
...
class TestOpenAIV1Caching:
    """OpenAI Chat Completions implicitly caches stable prefixes >= 1024 tokens.

    The v1 path must surface `usage.prompt_tokens_details.cached_tokens`
    so cost math doesn't double-bill the cached prefix at the base input
    rate.
    """

    @pytest.mark.asyncio
    async def test_v1_reports_cached_tokens_from_usage_details(self, monkeypatch):
        from ai_review.clients.openai.v1.schema import (
            OpenAIChatRequestSchema,
            OpenAIChatResponseSchema,
            OpenAIChoiceSchema,
            OpenAIMessageSchema as OpenAIV1Msg,
            OpenAIPromptTokensDetailsSchema,
            OpenAIUsageSchema as OpenAIV1Usage,
        )

        cfg = OpenAILLMConfig(
            meta=OpenAIMetaConfig(model="gpt-4o-mini"),
            provider=LLMProvider.OPENAI,
            http_client=OpenAIHTTPClientConfig(
                timeout=10,
                api_url=HttpUrl("https://api.openai.com"),
        ...
        class _FakeOpenAIV1Client:
            def __init__(self) -> None:
                self.requests: list[OpenAIChatRequestSchema] = []
            ...
            async def chat(self, request: OpenAIChatRequestSchema) -> OpenAIChatResponseSchema:
                ...
                return OpenAIChatResponseSchema(
                    usage=OpenAIV1Usage(
                        ...
                        prompt_tokens_details=OpenAIPromptTokensDetailsSchema(cached_tokens=220),
                    ),
                    choices=[OpenAIChoiceSchema(
                        message=OpenAIV1Msg(role="assistant", content="resp"),
                    )],
                )

        fake = _FakeOpenAIV1Client()
        monkeypatch.setattr(
            "ai_review.services.llm.openai.client.get_openai_v1_http_client",
            lambda: fake,
        )
        ...
        assert result.cache_read_tokens == 220
        # prompt_tokens must exclude the cached read so the base input
        # rate is not applied to bytes already billed at cache_read rate.
        assert result.prompt_tokens == 80
